...
pub const WINDOW_PERMISSION_HINT: &str = "Computer Use could not access a supported window list backend. Targeted window input requires session-bus access plus GNOME Shell Introspect, the Codex GNOME Shell extension, the COSMIC Wayland helper, KWin/Plasma DBus scripting, Niri IPC, Hyprland hyprctl, i3-msg, or generic X11/EWMH tools. On GNOME, run setup_window_targeting to install the extension backend.";
...
const DESCRIPTORS: &[BackendDescriptor] = &[
    ...
    BackendDescriptor {
        ...
        missing_hint: "On Niri, ensure `niri msg -j windows` works in the session.",
        can_exact_focus: true,
    },
    BackendDescriptor {
        id: HYPRLAND_BACKEND,
