  ...
  withFeatureConfig(["skill-invocation-policy"], (featuresRoot) => {
    ...
    assert.deepEqual(
      ...
      [
        ...
        ["feature:skill-invocation-policy:skill-invocation-policy-composer", "webview-asset", "opt-in"],
      ],
    );
  });
});
...
test("descriptors target the Skill card and main composer chunks", () => {
  assert.deepEqual(
    ...
    [
      ...
      ["skill-invocation-policy-composer", "webview-asset", "opt-in"],
    ],
  );
...
test("composer patch is byte-preserving when a required anchor drifts", () => {
  ...
  try {
    assert.equal(applySkillInvocationComposerPatch(source), source);
  } finally {
...
test("patch is fail-soft and byte-preserving when only part of the UI contract matches", () => {
  ...
  console.warn = (message) => warnings.push(message);
  try {
    assert.equal(applySkillInvocationPolicyPatch(partial), partial);
  } finally {
    console.warn = originalWarn;
  }
  assert.deepEqual(warnings, [
    "WARN: Could not resolve the Skill preview header actions - skipping Skill invocation policy patch",
  ]);
});
