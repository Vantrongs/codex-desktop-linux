...
test("browser-only sessions retain upstream initialization without a native client", () => {
  const configure = vm.runInNewContext(patch(selector) + ";configure", {
    pluginRoot: () => "/plugin",
    path: { default: require("node:path") },
    constants: { Il: "NODE_REPL_TRUSTED_SERVICES" },
  });
  const browser = configure({ surfaces: ["browser"] }).env;
  assert.equal(Object.hasOwn(browser, "NODE_REPL_JS_BANNER"), false);
  assert.equal(JSON.parse(browser.NODE_REPL_TRUSTED_SERVICES).sky, undefined);
  const native = configure({ surfaces: ["browser", "computer"] }).env;
