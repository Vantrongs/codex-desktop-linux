...
test("composer capture preserves insert-then-send when the action changes during insertion", async () => {
  const calls = [];
  const session = { action: "insert" };
  const source = "async function run(c,t,U,a,o,r){let l=c.trim();l.length>0?(o==null?_m.getInstance().dispatchMessage(`global-dictation-record-history-item`,{text:l}):o.setTranscript(l),r.performance.mark(`transcript_dispatched`),t.action===`send`?await a.onTranscriptSend(l):(await a.onTranscriptInsert(l),U.current===t&&U.current.action===`send`&&await a.onTranscriptSend(``))):a.onTranscriptCancel?.()}";
  const run = vm.runInNewContext(applyRecordReplayDictationTranscriptPatch(source) + ";run", {
    codexLinuxRecordReplayCaptureTranscript: (text, action) => { calls.push(["capture", text, action]); return true; },
  });
  await run(" hello ", session, { current: session }, {
    onTranscriptInsert: async text => { calls.push(["insert", text]); session.action = "send"; },
    onTranscriptSend: async text => calls.push(["send", text]),
  }, { setTranscript: text => calls.push(["persist", text]) }, { performance: { mark() {} } });
  assert.deepEqual(calls, [["capture", "hello", "insert"], ["persist", "hello"], ["insert", "hello"], ["send", ""]]);
});
