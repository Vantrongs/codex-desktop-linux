...
test("non-Debian package formats map the official runtime libraries", () => {
  ...
  for (const soname of ["libatspi.so.0", "libnotify.so.4", "libssl.so.3", "libusb-1.0.so.0", "libX11-xcb.so.1", "libtss2-esys.so.0", "libtss2-mu.so.0", "libtss2-tcti-device.so.0"]) {
