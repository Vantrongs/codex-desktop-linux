...
).find(({ id }) => id === "linux-renderer-crash-diagnostics");
const {
  ...
  try {
    ...
    webContentsListeners.get("console-message")({}, {
      message: `[codex-linux-renderer-breadcrumb]${JSON.stringify({
        v: 1,
        kind: "resize-observer-loop",
        routeIds: ["019fad43-fe09-73d0-9925-40fdd74c37c2"],
        observers: [{ id: 101, stack: "Error\\n at packaged (<url>:1:2)", targets: ["main"] }],
      })}`,
    });
    rendererPid = 0;
    ...
    assert.equal(records[0].renderer.url, "app://-");
    assert.equal(records[0].renderer.breadcrumbs.length, 4);
    ...
    assert.deepEqual(records[0].renderer.breadcrumbs[3].routeIds, [
      "019fad43-fe09-73d0-9925-40fdd74c37c2",
    ]);
    assert.doesNotMatch(
