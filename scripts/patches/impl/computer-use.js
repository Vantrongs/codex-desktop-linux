...
const COMPUTER_USE_AVAILABILITY_MARKER = "codexLinuxComputerUseAvailabilityV1";
const COMPUTER_USE_INSTALL_FLOW_MARKER = "codexLinuxComputerUseInstallFlowV1";
...
function applyCurrentComputerUseSettingsContract(currentSource) {
  ...
  let patchedSource = currentSource.replace(
    ...
    (match, availabilityVar, hookVar, hookArg, platformVar, platformHookVar, nextVar, offset) => {
      ...
      return `let ${availabilityVar}=${hookVar}(${hookArg}),{platform:${platformVar}}=${platformHookVar}();/*${COMPUTER_USE_AVAILABILITY_MARKER}*/${platformVar}===\`linux\`&&(${availabilityVar}={...${availabilityVar},available:!0,isFetching:!1,isLoading:!1});let ${nextVar}=`;
    },
  );
  ...
  const currentCardPattern =
    /let ([A-Za-z_$][\w$]*)=([A-Za-z_$][\w$]*)\(([A-Za-z_$][\w$]*),([A-Za-z_$][\w$]*)\),([A-Za-z_$][\w$]*)=([A-Za-z_$][\w$]*)\(\3\),([^;]{1,400}?),([A-Za-z_$][\w$]*);/g;
  patchedSource = patchedSource.replace(
    currentCardPattern,
    (
      match,
      ...
      const lookback = patchedSource.slice(Math.max(0, offset - 1400), offset);
      const nextSource = patchedSource.slice(offset + match.length, offset + match.length + 700);
      const platformVar = lookback.match(
        /\{computerUseAvailability:[A-Za-z_$][\w$]*,platform:([A-Za-z_$][\w$]*)\}=/,
      )?.[1];
      const selectorPattern = new RegExp(
        String.raw`${escapeRegExp(selectedPluginVar)}=[A-Za-z_$][\w$]*\(${escapeRegExp(pluginsQueryVar)}\.availablePlugins,([A-Za-z_$][\w$]*),${escapeRegExp(marketplacePathVar)}\)`,
      );
      const pluginNameVar = nextSource.match(selectorPattern)?.[1];
      if (platformVar == null || pluginNameVar == null) {
        return match;
      }
      cardChanged = true;
      return `${match}${platformVar}===\`linux\`&&!${pluginsQueryVar}.availablePlugins.some(e=>e.plugin?.name===${pluginNameVar}||e.plugin?.id?.split(\`@\`)[0]===${pluginNameVar})&&(${pluginsQueryVar}={...${pluginsQueryVar},availablePlugins:[...${pluginsQueryVar}.availablePlugins,{marketplaceName:\`openai-bundled\`,marketplacePath:${marketplacePathVar},logoPath:new URL(\`computer-use-plugin-icon-linux.png\`,import.meta.url).href,logoDarkPath:new URL(\`computer-use-plugin-icon-linux.png\`,import.meta.url).href,plugin:{id:${pluginNameVar},name:${pluginNameVar},installed:!0,enabled:!0}}]});`;
    },
  );
  ...
  if (currentSource.includes("plugin detail query requires pluginName")) {
    ...
    const patchedSource = currentSource.replace(
      ...
      (match, gateVar, gateValueVar, nextVar, offset) => {
        ...
        changed = true;
        return `let ${gateVar}=${gateValueVar}&&${pluginNameVar}!==\`computer-use\`,${nextVar};/*${COMPUTER_USE_INSTALL_FLOW_MARKER}*/`;
      },
    );

    if (
      changed &&
      markerPattern.test(patchedSource) &&
      patchedSource.includes(COMPUTER_USE_INSTALL_FLOW_MARKER)
    ) {
      return patchedSource;
    }
