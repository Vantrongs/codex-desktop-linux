...
test("default core patch descriptors are grouped and unique", () => {
  ...
  assert.equal(
    computerUseHostPlatform.pattern.test(
      "app-initial~artifact-tab-content.electron~notebook-preview-panel~app-main~settings-command-~cajo70vh-current.js",
    ),
    true,
  );
...
function currentTrayLifecycleBundleFixture() {
  return [
    ...
    "let oe=()=>{};(E||process.platform===`linux`)&&oe();",
  ].join("");
}
...
test("starts the native Linux tray only when the persisted tray setting allows it", () => {
  const source = `${currentMainBundlePrefix}${trayBundleFixture()}`;
  const patched = applyPatchTwice(
    applyLinuxTrayPatch,
    applyLinuxQuitGuardPatch(source),
  ...
  const startupMatch = patched.match(
    /\(E\|\|process\.platform===`linux`&&\(typeof codexLinuxIsTrayEnabled!==`function`\|\|codexLinuxIsTrayEnabled\(\)\)\)&&oe\(\);/,
  );

  assert.notEqual(startupMatch, null);
  assert.doesNotMatch(patched, /\(E\|\|process\.platform===`linux`\)&&oe\(\);/);

  const runStartup = (traySetting) => {
    let calls = 0;
    const execute = new Function(
      "process",
      "codexLinuxIsTrayEnabled",
      "oe",
      "E",
      `${startupMatch[0]}return null;`,
    );
    execute({ platform: "linux" }, traySetting, () => {
      calls += 1;
    }, false);
    return calls;
  };

  assert.equal(runStartup(() => false), 0);
  assert.equal(runStartup(() => true), 1);
  assert.equal(runStartup(undefined), 1);
});
...
test("rejects Linux tray setting-gate drift without partial rewrites", () => {
  const source = currentTrayLifecycleBundleFixture().replace(
    "let oe=()=>{};(E||process.platform===`linux`)&&oe();",
    "",
  );
  ...
  assert.equal(patched, source);
  assert.deepEqual(warnings, [
    "WARN: Could not resolve the current Linux tray startup — skipping Linux tray setting gate patch",
  ]);
});
...
test("Computer Use availability descriptor matches the current settings bundle name", () => {
  ...
  const installDescriptor = descriptors.find((entry) => entry.id === "linux-computer-use-install-flow");
  ...
  assert.match(
    "app-initial~avatarOverlayCompositionSurface~artifact-tab-content.electron~notebook-preview-~iaq4jiqv-DzB2NxMS.js",
    installDescriptor.pattern,
  );
  assert.doesNotMatch("plugin-detail-page-BJPLeWFT.js", installDescriptor.pattern);
});
...
test("enables the Electron 42 Computer Use settings card contract on Linux", () => {
  const source =
    "function Ht(){let e=cache(24),{selectedHostId:t}=host(),i={hostId:t};" +
    "let a=ae(i),{platform:o}=ce(),s=kind(t);let f=jsx(Wt,{computerUseAvailability:a,platform:o});" +
    "let p=a.available?jsx(AllowedApps,{}):null;return jsx(Page,{children:[f,p]})}" +
    "function Wt(e){let t=cache(35),{computerUseAvailability:n,platform:i}=e,{selectedHostId:s}=host(),g=[];" +
    "let _=de(s,g),v=ht(s),y=flag(Ue),b=flag(Ve),x;" +
    "t[5]!==v||t[6]!==_.availablePlugins?(x=K(_.availablePlugins,fn,v),t[5]=v,t[6]=_.availablePlugins,t[7]=x):x=t[7];" +
    "let S=x;return n.available&&S!=null?S:null}";
  ...
  assert.match(
    patched,
    /i===`linux`&&[A-Za-z_$][\w$]*BundledMarketplaceDonor!=null&&!_\.availablePlugins\.some/,
  );
  assert.match(
    patched,
    /marketplaceName:`openai-bundled`,marketplacePath:[A-Za-z_$][\w$]*BundledMarketplaceDonor\.marketplacePath/,
  );
  assert.match(patched, new RegExp(COMPUTER_USE_AVAILABILITY_MARKER));
});
...
test("loads current Computer Use plugin details on Linux despite the upstream availability gate", () => {
  ...
  assert.match(patched, new RegExp(COMPUTER_USE_INSTALL_FLOW_MARKER));
  assert.doesNotMatch(patched, /let g=m,_;/);
});

test("restores required markers on current structurally patched Computer Use assets", () => {
  const settingsSource =
    "function Ht(){let e=cache(24),{selectedHostId:t}=host(),n=data(t),i={hostId:t};" +
    "let a=useAvailability(i),{platform:o}=usePlatform(),s=hostKind(t)===`local`,c=flag(`188145323`);" +
    "let f=jsx(Settings,{computerUseAvailability:a,platform:o});" +
    ...
    "let g=[];let _=usePlugins(s,g),v=useMarketplacePath(s),y=useFlag(firstFlag),b=useFlag(secondFlag),x;" +
    "x=selectPlugin(_.availablePlugins,computerUsePluginName,v);return x}";
  const structurallyPatchedSettings = applyLinuxComputerUseRendererAvailabilityPatch(
    settingsSource,
  ).replace(
    `/*${COMPUTER_USE_AVAILABILITY_MARKER}*/`,
    "",
  );
  const markedSettings = applyLinuxComputerUseRendererAvailabilityPatch(structurallyPatchedSettings);
  assert.notEqual(markedSettings, structurallyPatchedSettings);
  assert.equal((markedSettings.match(new RegExp(COMPUTER_USE_AVAILABILITY_MARKER, "g")) ?? []).length, 1);
  ...
  const installSource =
    ...
    "let query=async()=>{if(i==null)throw Error(`plugin detail query requires pluginName`);return read(`read-plugin`,{hostId:l,pluginName:i})};" +
    "return useQuery({queryFn:query,enabled:x})}";
  const structurallyPatchedInstall = applyLinuxComputerUseInstallFlowPatch(
    installSource,
  ).replace(
    `/*${COMPUTER_USE_INSTALL_FLOW_MARKER}*/`,
    "",
  );
  const markedInstall = applyLinuxComputerUseInstallFlowPatch(structurallyPatchedInstall);
  assert.notEqual(markedInstall, structurallyPatchedInstall);
  assert.equal((markedInstall.match(new RegExp(COMPUTER_USE_INSTALL_FLOW_MARKER, "g")) ?? []).length, 1);
});
...
test("webview required markers reject decoy assets atomically and recognize a complete prior patch", () => {
  const tempApp = fs.mkdtempSync(path.join(os.tmpdir(), "codex-required-marker-assets-"));
  try {
    const assetsDir = path.join(tempApp, "webview", "assets");
    fs.mkdirSync(assetsDir, { recursive: true });
    const firstAsset = path.join(assetsDir, "demo-a.js");
    const secondAsset = path.join(assetsDir, "demo-b.js");
    fs.writeFileSync(firstAsset, "first asset");
    fs.writeFileSync(secondAsset, "decoy asset");

    const markerA = "codexMarkerContractA";
    const markerB = "codexMarkerContractB";
    const descriptors = normalizePatchDescriptors([
      {
        id: "feature:marker-contract:demo",
        featureId: "marker-contract",
        sourceKind: "feature",
        phase: "webview-asset",
        ciPolicy: "opt-in",
        pattern: /^demo-.*\.js$/,
        requiredMarkers: [markerA, markerB],
        apply: (source) => source.includes(markerA)
          ? source
          : source.replace("first", `first ${markerA}`),
      },
    ]);

    const failedReport = createPatchReport();
    failedReport.enabledFeatures = ["marker-contract"];
    const { warnings } = captureWarns(() =>
      applyWebviewAssetPatchDescriptors(tempApp, descriptors, {}, failedReport));
    assert.equal(failedReport.patches[0].status, "skipped-optional");
    assert.deepEqual(failedReport.patches[0].markerCounts, {
      [markerA]: 1,
      [markerB]: 0,
    });
    assert.equal(failedReport.patches[0].attemptedChanged, 1);
    assert.equal(enabledFeatureFailuresFromReport(failedReport).length, 1);
    assert.match(warnings[0], /did not produce exactly one copy/);
    assert.equal(fs.readFileSync(firstAsset, "utf8"), "first asset");
    assert.equal(fs.readFileSync(secondAsset, "utf8"), "decoy asset");

    fs.writeFileSync(firstAsset, `first asset ${markerA}`);
    fs.writeFileSync(secondAsset, `second asset ${markerB}`);
    const completeReport = createPatchReport();
    completeReport.enabledFeatures = ["marker-contract"];
    applyWebviewAssetPatchDescriptors(tempApp, descriptors, {}, completeReport);
    assert.equal(completeReport.patches[0].status, "already-applied");
    assert.equal(enabledFeatureFailuresFromReport(completeReport).length, 0);
  } finally {
    fs.rmSync(tempApp, { recursive: true, force: true });
  }
});

test("semantic webview required markers reject an incomplete selected asset atomically", () => {
  const tempApp = fs.mkdtempSync(path.join(os.tmpdir(), "codex-semantic-marker-assets-"));
  try {
    const assetsDir = path.join(tempApp, "webview", "assets");
    fs.mkdirSync(assetsDir, { recursive: true });
    const selectedAsset = path.join(assetsDir, "demo-selected.js");
    const decoyAsset = path.join(assetsDir, "demo-decoy.js");
    fs.writeFileSync(selectedAsset, "selected contract");
    fs.writeFileSync(decoyAsset, "decoy contract");

    const markerA = "codexSemanticMarkerA";
    const markerB = "codexSemanticMarkerB";
    const descriptors = normalizePatchDescriptors([
      {
        id: "semantic-marker-contract",
        phase: "webview-asset",
        ciPolicy: "required-upstream",
        pattern: /^demo-.*\.js$/,
        assetMatch: (source) => source.includes("selected"),
        requiredMarkers: [markerA, markerB],
        apply: (source) => `${source} ${markerA}`,
      },
    ]);

    const report = createPatchReport();
    const { warnings } = captureWarns(() =>
      applyWebviewAssetPatchDescriptors(tempApp, descriptors, {}, report));

    assert.equal(report.patches[0].status, "failed-required");
    assert.equal(report.patches[0].assetName, "demo-selected.js");
    assert.deepEqual(report.patches[0].markerCounts, {
      [markerA]: 1,
      [markerB]: 0,
    });
    assert.equal(report.patches[0].attemptedChanged, 1);
    assert.match(warnings[0], /did not produce exactly one copy/);
    assert.equal(fs.readFileSync(selectedAsset, "utf8"), "selected contract");
    assert.equal(fs.readFileSync(decoyAsset, "utf8"), "decoy contract");
  } finally {
    fs.rmSync(tempApp, { recursive: true, force: true });
  }
});
