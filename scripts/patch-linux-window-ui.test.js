...
test("Computer Use availability descriptor matches the current settings bundle name", () => {
  const [descriptor, installDescriptor] = require("./patches/core/all-linux/webview/computer-use-ui/patch.js");
...
test("enables the Electron 42 Computer Use settings card contract on Linux", () => {
  ...
  assert.match(patched, /i===`linux`&&!_\.availablePlugins\.some/);
  assert.match(patched, /marketplaceName:`openai-bundled`,marketplacePath:v/);
  assert.match(patched, new RegExp(COMPUTER_USE_AVAILABILITY_MARKER));
});
...
test("backfills fixed markers onto complete pre-marker Computer Use patches", () => {
  const settingsSource =
    "function Ht(){let a=availability(arg),{platform:o}=platform(),s=next;" +
    "let d=jsx(Settings,{computerUseAvailability:a,platform:o});let m=a.available?jsx(Apps,{}):null}" +
    "function Wt(e){let{computerUseAvailability:n,platform:i}=e;let g=plugins(host,empty),v=path(host),b=flag(arg);" +
    "i===`linux`&&!g.availablePlugins.some(e=>e.plugin?.name===pluginName||e.plugin?.id?.split(`@`)[0]===pluginName)&&(g={...g,availablePlugins:[...g.availablePlugins,{marketplaceName:`openai-bundled`,marketplacePath:v,plugin:{id:pluginName,name:pluginName}}]});" +
    "let x;x=select(g.availablePlugins,pluginName,v);return x}";
  const structurallyPatchedSettings = settingsSource.replace(
    "let a=availability(arg),{platform:o}=platform(),s=next;",
    "let a=availability(arg),{platform:o}=platform();o===`linux`&&(a={...a,available:!0,isFetching:!1,isLoading:!1});let s=next;",
  );
  ...
  const structurallyPatchedInstall =
    "function detail(e){let{pluginName:i}=e,f=gate(i);let p=f&&i!==`computer-use`,m;" +
    "let h=availability({enabled:p}),v=!p||h.available;" +
    "let query=()=>{if(i==null)throw Error(`plugin detail query requires pluginName`);" +
    "return read(`read-plugin`,{pluginName:i})};return query}";
