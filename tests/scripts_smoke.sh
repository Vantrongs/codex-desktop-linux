...
PY
    ...
    local clean_tool_path=""
    local tool_dir
    local selected_cli
    while IFS= read -r tool_dir; do
        [ -n "$tool_dir" ] || continue
        [ ! -x "$tool_dir/codex" ] || continue
        if [ -z "$clean_tool_path" ]; then
            clean_tool_path="$tool_dir"
        else
            clean_tool_path="$clean_tool_path:$tool_dir"
        fi
    done < <(printf '%s\n' "$HOST_TOOL_PATH" | tr ':' '\n')
    [ -n "$clean_tool_path" ] || fail "Could not build a Codex-free tool PATH"
...
JS
    ...
    assert_not_contains "$extracted/.vite/build/main-test.js" '(E||process.platform===`linux`)&&oe();'
...
const startupSnippet = source.match(/\(E\|\|process\.platform===`linux`&&\(typeof codexLinuxIsTrayEnabled!==`function`\|\|codexLinuxIsTrayEnabled\(\)\)\)&&oe\(\);/)?.[0];
if (!closeSnippet || !startupSnippet) {
  throw new Error("Could not extract patched Linux tray behavior");
}

function runTrayStartup(traySetting) {
  let calls = 0;
...
if (runTrayStartup(() => false) !== 0) {
  throw new Error("disabled Linux tray setting should skip tray startup");
}
if (runTrayStartup(() => true) !== 1 || runTrayStartup(undefined) !== 1) {
  throw new Error("enabled or unavailable Linux tray setting should preserve tray startup");
}
...
NODE
    ...
    assert_occurrence_count "$extracted/.vite/build/main-test.js" 'typeof codexLinuxIsTrayEnabled!==`function`||codexLinuxIsTrayEnabled()' '1'
...
test_linux_computer_use_ui_opt_in_smoke() {
    ...
    local host_platform_asset="$extracted/webview/assets/app-initial~artifact-tab-content.electron~notebook-preview-panel~app-main~settings-command-~cajo70vh-test.js"
