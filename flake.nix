    ...
    flake-utils.lib.eachSystem [ "x86_64-linux" "aarch64-linux" ] (system:
      let
        ...
        helperWorkspaceSource = lib.cleanSourceWith {
          # A cleanSourceWith result has a store-path identity, while nested
          # filters still receive paths from the original source tree.  Using
          # sourceRoot here therefore made every relative-path comparison fail
          # and produced an empty helper workspace.  Filter the repository
          # directly and retain only the workspace inputs needed by Cargo.
          src = ./.;
          filter = path: type:
            let
